-- ===================================================
-- 🎮 DATOS DE PRUEBA GAMING - COLUMNAS REALES
-- ===================================================

-- ⚠️ USANDO LAS COLUMNAS REALES DE LAS TABLAS
...
-- ===================================================
-- 💰 DATOS DE MONEDAS (COLUMNAS REALES)
-- ===================================================
...
INSERT INTO monedas_usuario (
    ...
    monedas_disponibles,    -- ✅ Disponibles para gastar
...
-- Usuario avanzado: Muchas monedas
((SELECT id FROM perfiles LIMIT 1 OFFSET 0), 2500, 1000, 1500, 800, 600, 500, 600, 15, NOW() - INTERVAL '1 day', NOW() - INTERVAL '3 days'),
-- Usuario intermedio: Monedas medias
((SELECT id FROM perfiles LIMIT 1 OFFSET 1), 1200, 400, 800, 400, 200, 300, 300, 8, NOW() - INTERVAL '2 days', NOW() - INTERVAL '1 week'),
-- Usuario principiante: Pocas monedas
((SELECT id FROM perfiles LIMIT 1 OFFSET 2), 200, 50, 150, 100, 50, 0, 50, 3, NOW() - INTERVAL '1 week', NOW() - INTERVAL '2 weeks')
ON CONFLICT (usuario_id) DO UPDATE SET
    monedas_totales = EXCLUDED.monedas_totales,
    monedas_gastadas = EXCLUDED.monedas_gastadas,
    monedas_disponibles = EXCLUDED.monedas_disponibles,
...
DO $$
BEGIN
    RAISE NOTICE '🎮 ¡DATOS GAMING INSERTADOS CON COLUMNAS REALES!';
