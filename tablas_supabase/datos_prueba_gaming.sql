-- ===========================================
-- 🎮 DATOS DE PRUEBA PARA SISTEMA GAMING
-- ===========================================

-- Nota: Este archivo crea datos de prueba para el sistema de gamificación
-- Ejecutar después de tener las tablas gaming creadas y algunos usuarios en perfiles

-- ===========================================
-- 📊 DATOS DE EXPERIENCIA DE USUARIOS
-- ===========================================

INSERT INTO experiencia_usuario (usuario_id, nivel, xp_actual, xp_total, xp_siguiente_nivel, xp_cursos, xp_simulador, xp_comunidad, xp_logros, racha_dias, racha_maxima, ultima_sesion) VALUES
-- Usuario nivel alto (ejemplo)
...
ON CONFLICT (usuario_id, tipo_ranking) DO UPDATE SET
...
-- ===========================================
-- 📈 DATOS DE ESTADÍSTICAS DE USUARIOS
-- ===========================================
...
-- Usuario avanzado
((SELECT id FROM perfiles LIMIT 1 OFFSET 0), 45, 1200, NOW() - INTERVAL '3 months', NOW() - INTERVAL '2 hours',
 98, 95, 3000, 2850, 5, 12, 45,
 15, 125, 45, 8, 3, 3, 2, 0,
 7, 15, 45, 1, 1, 8, NOW()),

-- Usuario intermedio
((SELECT id FROM perfiles LIMIT 1 OFFSET 1), 28, 600, NOW() - INTERVAL '2 months', NOW() - INTERVAL '1 day',
 92, 87, 1800, 1566, 3, 8, 28,
 8, 45, 22, 4, 2, 2, 0, 0,
 3, 8, 28, 2, 3, 4, NOW()),

-- Usuario principiante
((SELECT id FROM perfiles LIMIT 1 OFFSET 2), 12, 180, NOW() - INTERVAL '1 month', NOW() - INTERVAL '3 days',
...
-- ===========================================
-- 🎮 DATOS DE SESIONES DEL SIMULADOR
-- ===========================================
...
-- Sesión del usuario avanzado
((SELECT id FROM perfiles LIMIT 1 OFFSET 0), 30, 1800, NOW() - INTERVAL '2 hours', NOW() - INTERVAL '90 minutes',
 120, 114, 6, 95, 120, ARRAY['C Major', 'G Major'], ARRAY['C', 'F', 'G'], ARRAY['La Gota Fría'], 'FBE',
 'cancion', 250, ARRAY[], 15, 15, '{"modo_practica": "cancion_completa", "dificultad": "avanzado"}'),

-- Sesión del usuario intermedio  
((SELECT id FROM perfiles LIMIT 1 OFFSET 1), 20, 1200, NOW() - INTERVAL '1 day', NOW() - INTERVAL '23 hours',
 80, 70, 10, 87, 100, ARRAY['C Major'], ARRAY['C', 'F'], ARRAY['El Provinciano'], 'FBE',
 'leccion', 180, ARRAY[], 8, 8, '{"modo_practica": "leccion_guiada", "dificultad": "intermedio"}'),

-- Sesión del usuario principiante
((SELECT id FROM perfiles LIMIT 1 OFFSET 2), 15, 900, NOW() - INTERVAL '3 days', NOW() - INTERVAL '3 days' + INTERVAL '15 minutes',
 50, 36, 14, 72, 80, ARRAY['C Major'], ARRAY['C'], ARRAY[], 'FBE',
 'escala', 120, ARRAY[], 3, 3, '{"modo_practica": "ejercicios_basicos", "dificultad": "principiante"}');

-- ===========================================
-- 🎯 DATOS DE LOGROS DE USUARIOS
-- ===========================================
...
((SELECT id FROM perfiles LIMIT 1 OFFSET 0), (SELECT id FROM logros_sistema WHERE nombre = 'Precisión Experta' LIMIT 1), true, 100, 100, 100, '{"precision_maxima": 98}', NOW() - INTERVAL '2 weeks', NOW() - INTERVAL '1 month', NOW() - INTERVAL '2 hours'),

-- Logros del usuario intermedio
...
((SELECT id FROM perfiles LIMIT 1 OFFSET 1), (SELECT id FROM logros_sistema WHERE nombre = 'Constancia Semanal' LIMIT 1), true, 100, 100, 100, '{"racha_dias": 8}', NOW() - INTERVAL '3 weeks', NOW() - INTERVAL '1 month', NOW() - INTERVAL '1 day'),

-- Logros del usuario principiante
...
BEGIN
  ...
  RAISE NOTICE '📊 Experiencia: % usuarios', (SELECT COUNT(*) FROM experiencia_usuario);
  RAISE NOTICE '🏆 Rankings: % entradas', (SELECT COUNT(*) FROM ranking_global);
  RAISE NOTICE '📈 Estadísticas: % usuarios', (SELECT COUNT(*) FROM estadisticas_usuario);
  RAISE NOTICE '🎮 Sesiones: % registradas', (SELECT COUNT(*) FROM sesiones_simulador);
  RAISE NOTICE '🏅 Logros de usuarios: % conseguidos', (SELECT COUNT(*) FROM logros_usuario WHERE conseguido = true);
  RAISE NOTICE '💰 Monedas: % usuarios con monedas', (SELECT COUNT(*) FROM monedas_usuario);
END $$; 