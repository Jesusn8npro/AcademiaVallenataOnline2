<script lang="ts">
    ...
    async function cargarDatos() {
        try {
            // Cargar paquete
            const resultadoPaquete = await obtenerPaquetePorId(paqueteId);
            if (resultadoPaquete.success) {
                paquete = resultadoPaquete.data;
            }
            ...
            // Cargar tutoriales del paquete
            const resultadoTutoriales = await obtenerTutorialesPaquete(paqueteId);
            if (resultadoTutoriales.success) {
                tutorialesSeleccionados = resultadoTutoriales.data.map((item: any) => ({
                    ...item.tutoriales,  // Cambié de item.tutorial a item.tutoriales
                    orden: item.orden,
                    paquete_item_id: item.id
                }));
            }
        } catch (err: any) {
    ...
    async function guardar() {
        ...
        try {
            guardando = true;
            const resultado = await actualizarPaquete(paqueteId, paquete);
            
            if (resultado.success) {
                exito = 'Paquete actualizado exitosamente';
                setTimeout(() => goto('/administrador/paquetes'), 1500);
            } else {
                error = resultado.error || 'Error actualizando paquete';
            }
        } catch (err: any) {
    ...
    async function agregarTutorial(tutorial: any) {
        if (!tutorialesSeleccionados.find(t => t.id === tutorial.id)) {
            try {
                // Agregar a la base de datos
                const resultado = await agregarTutorialAPaquete({
                    paquete_id: paqueteId,
                    tutorial_id: tutorial.id,
                    orden: tutorialesSeleccionados.length + 1,
                    incluido: true,
                ...
                if (resultado.success) {
                    ...
                    tutorialesSeleccionados = [...tutorialesSeleccionados, {
                        ...tutorial,
                        orden: tutorialesSeleccionados.length + 1,
                        paquete_item_id: resultado.data?.id
                    }];
                    calcularPreciosSugeridos(); // Recalcular precios
                    error = ''; // Limpiar errores anteriores
                } else {
                    error = 'Error agregando tutorial: ' + resultado.error;
                }
            } catch (err: any) {
                error = 'Error: ' + err.message;
            }
    ...
    async function removerTutorial(tutorialId: string) {
        try {
            // Remover de la base de datos
            const resultado = await removerTutorialDePaquete(paqueteId, tutorialId);
            ...
            if (resultado.success) {
                ...
                error = 'Error removiendo tutorial: ' + resultado.error;
            }
        } catch (err: any) {
            error = 'Error: ' + err.message;
        }
    ...
    function calcularPreciosSugeridos() {
        if (tutorialesSeleccionados.length > 0) {
            ...
            // Sugerir con 30% descuento si no hay precio normal
            if (!paquete.precio_normal || paquete.precio_normal === 0) {
                paquete.precio_normal = Math.round(total * 0.7);
            }
            
            // Sugerir precio rebajado con 40% descuento si no hay precio rebajado
        ...
        <form on:submit|preventDefault={guardar} class="form">
            ...
            <section class="card">
                ...
                <div class="fields">
                    <div class="field">
                        <label>Título</label>
                        <input type="text" bind:value={paquete.titulo} required />
                    </div>
                    <div class="field">
                ...
                <div class="fields">
                    <div class="row">
                        <div class="field">
                            ...
                            <input type="text" bind:value={paquete.categoria} />
                        </div>
                        <div class="field">
                            <label>Nivel</label>
            ...
            <section class="card">
                ...
                {#if tutorialesSeleccionados.length === 0}
                    <div class="empty">
                        <p>No hay tutoriales en este paquete</p>
...
<style>
    ...
    /* Estilos para previsualización de precios */
    .price-preview {
        display: flex;
        align-items: center;
        gap: 0.5rem;
    ...
    .price-preview .price-current {
        font-size: 1.25rem;
        font-weight: bold;
        color: #28a745;
    }

    .price-preview .price-original {
        text-decoration: line-through;
        color: #999;
        font-size: 1rem;
    }
